  ...
  <main class="main">
    <div class="main-inner">
      ...
      <div class="toolbar">
        <form action="{{ route('admin.pengguna') }}" method="GET" style="display: flex; flex: 1; align-items: center;">
          ...
          <div class="search-wrap" style="width: 100%;">
            ...
            <input class="search-input" type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama atau email pengguna..." />
          </div>
        </form>
        ...
        <div class="toolbar-right">
          ...
          <div class="dropdown-wrap" id="sortDrop">
            <div class="dropdown-btn" id="sortBtn" onclick="toggleDrop('sortDrop')">
              <span id="sortLabel">Urutkan: {{ request('sort') == 'terlama' ? 'Akun Terlama' : 'Akun Terbaru' }}</span>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg>
            </div>
            <div class="dropdown-menu" id="sortMenu">
              <div class="drop-item {{ request('sort', 'terbaru') != 'terlama' ? 'selected' : '' }}" onclick="window.location.href='{{ route('admin.pengguna', ['sort' => 'terbaru', 'q' => request('q')]) }}'">Akun Terbaru</div>
              <div class="drop-item {{ request('sort') == 'terlama' ? 'selected' : '' }}" onclick="window.location.href='{{ route('admin.pengguna', ['sort' => 'terlama', 'q' => request('q')]) }}'">Akun Terlama</div>
            </div>
          </div>
        </div>
      </div>
            ...
            @forelse($pengguna as $u)
              <tr>
                ...
                <td>
                  ...
                  @else
                    <span class="status-badge suspended">Tidak Aktif</span>
                  @endif
                </td>
                <td>
                  <div class="action-buttons">
                    ...
                    <button class="act-btn edit" onclick="openToggleActiveModal({{ $u->id }}, '{{ addslashes($u->nama) }}', {{ $u->is_active ? 1 : 0 }})" title="{{ $u->is_active ? 'Nonaktifkan Akun' : 'Aktifkan Akun' }}" style="background: rgba(249, 115, 22, 0.08); color: var(--orange); border: 1px solid rgba(249, 115, 22, 0.2); padding: 6px 8px;">
                    ...
                    <button class="act-btn delete" onclick="openDeleteModal({{ $u->id }}, '{{ addslashes($u->nama) }}')" title="Hapus Akun">
      ...
      <div class="pagination-bar" style="margin-top: 24px;">
        <span class="pagination-info">Menampilkan {{ $pengguna->firstItem() ?? 0 }}-{{ $pengguna->lastItem() ?? 0 }} dari {{ $pengguna->total() }} pengguna</span>
...
<div class="modal-overlay" id="toggleActiveModalOverlay">
  <div class="modal delete-modal" id="toggleActiveModalBox" style="border-color: rgba(249, 115, 22, 0.3);">
    ...
    <div class="delete-modal-title" id="toggleActiveModalTitle">Ubah Status Akun?</div>
    <div class="delete-modal-text" id="toggleActiveModalText">Apakah Anda yakin ingin mengubah status aktif pengguna <strong id="toggleActiveUserName">Nama</strong>?</div>
    <div class="delete-modal-actions">
      <button class="btn-cancel" onclick="closeToggleActiveModal()">Batal</button>
      <button class="btn-confirm-delete" id="btnConfirmToggleActive" onclick="confirmToggleActive()" style="background: var(--orange); box-shadow: 0 4px 12px rgba(249, 115, 22, 0.3);">Ya, Ubah Status</button>
    </div>
  </div>
</div>
