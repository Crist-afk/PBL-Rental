    ...
    <main class="flex-grow pt-32 pb-20 px-4 sm:px-6 max-w-7xl mx-auto w-full flex flex-col gap-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-8">
            ...
            <section class="glass-card rounded-[2.5rem] border-2 border-dark-chocolate/10 p-4 shadow-xl flex items-center justify-center">
                <div class="w-full h-[400px] md:h-[500px] bg-dark-chocolate/10 rounded-[2rem] flex flex-col items-center justify-center relative overflow-hidden group">
                    @if(isset($product['image']))
                        <img src="{{ $product['image'] }}" alt="{{ $product['title'] }}" class="w-full h-full object-cover transition duration-500 group-hover:scale-105">
                    @else
                        <div class="flex flex-col items-center text-dark-chocolate/40">
                            <i class="fa-solid fa-image text-6xl mb-4"></i>
                            <span class="font-bold">Gambar belum tersedia</span>
                        </div>
                    @endif
                </div>
            </section>
            ...
            <section class="glass-card rounded-[2.5rem] border-2 border-dark-chocolate/10 p-6 md:p-8 shadow-xl flex flex-col">
                <span class="mb-2 block text-xs font-black uppercase tracking-[0.35em] text-aloewood">{{ $product['category'] }}</span>
                <h1 class="text-4xl md:text-5xl font-extrabold text-dark-chocolate mb-2">{{ $product['title'] }}</h1>
                
                <div class="flex flex-wrap items-center gap-4 mb-6 pb-6 border-b border-dark-chocolate/10">
                    <span class="text-3xl font-black text-sakura">Rp {{ number_format($product['price'], 0, ',', '.') }} <span class="text-lg font-medium text-dark-chocolate/60">/ 3 Hari</span></span>
                ...
                <div class="space-y-4 mb-8 flex-grow">
                    ...
                    <p class="text-dark-chocolate/80 text-sm font-medium leading-relaxed">
                        {{ $product['description'] ?? 'Belum ada deskripsi untuk kostum ini.' }}
                    </p>
                    ...
                    <ul class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm font-medium text-dark-chocolate/80">
                        <li class="flex items-center gap-2"><div class="w-5 h-5 rounded-full bg-sakura/20 flex items-center justify-center"><i class="fa-solid fa-check text-[10px] text-sakura"></i></div> Set Kostum Lengkap</li>
                        <li class="flex items-center gap-2"><div class="w-5 h-5 rounded-full bg-sakura/20 flex items-center justify-center"><i class="fa-solid fa-check text-[10px] text-sakura"></i></div> Aksesoris Dasar</li>
                        ...
                        <div class="flex flex-wrap gap-3">
                            @foreach(['S', 'M', 'L', 'XL'] as $size)
                                <label class="cursor-pointer">
                                    ...
                                    <div class="w-12 h-12 flex items-center justify-center rounded-xl border-2 border-dark-chocolate/20 font-bold text-dark-chocolate peer-checked:border-sakura peer-checked:bg-sakura peer-checked:text-dark-chocolate transition shadow-sm hover:border-sakura">{{ $size }}</div>
                                </label>
                            @endforeach
                        </div>
                    </div>
